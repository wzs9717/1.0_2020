	...
	public void OnWillRenderObject()
	{
		...
		if (current.stereoEnabled)
		{
			if (current.stereoTargetEye == StereoTargetEyeMask.Both)
			{
				...
				{
					reflectionCamera.transform.position = current.transform.parent.TransformPoint(InputTracking.GetLocalPosition((XRNode)1));
					reflectionCamera.transform.rotation = current.transform.parent.rotation * InputTracking.GetLocalRotation((XRNode)1);
				...
				{
					reflectionCamera.transform.position = current.transform.parent.TransformPoint(InputTracking.GetLocalPosition((XRNode)0));
					reflectionCamera.transform.rotation = current.transform.parent.rotation * InputTracking.GetLocalRotation((XRNode)0);
