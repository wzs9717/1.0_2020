...
public class OVRManager : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			//if (XRSettings.get_renderScale() < maxRenderScale)
			//{
			//	XRSettings.set_renderScale(maxRenderScale);
			//}
			//else
			//{
			//	maxRenderScale = Mathf.Max(maxRenderScale, XRSettings.get_renderScale());
			//}
			//float min = minRenderScale / XRSettings.get_renderScale();
			//float value = OVRPlugin.GetEyeRecommendedResolutionScale() / XRSettings.get_renderScale();
			//value = Mathf.Clamp(value, min, 1f);
			//XRSettings.set_renderViewportScale(value);
		}
